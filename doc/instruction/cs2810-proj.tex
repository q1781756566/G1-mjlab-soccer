...
\begin{itemize}
    \item \texttt{POST /act} --- Receives an observation and returns an action.
    \begin{verbatim}
...
\begin{itemize}
    ...
    Teams deploy their policy servers and the tournament is conducted using \texttt{scripts/compete.py}. For each pair of teams (Team A and Team B), both directions are evaluated:
    \begin{itemize}
...
The script loads two G1 robots into a single MuJoCo scene, sends observations to each team's API (or local policy), concatenates their actions, and reports head-to-head statistics (goals scored, blocks). A reference API server implementation is provided as \texttt{scripts/api\_server.py}.
